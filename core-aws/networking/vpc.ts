/**
 * VPC 172.16.0.0/20: subnets, IGW, NAT, route tables, flow logs.
 */
import * as aws from "@pulumi/aws";

import { namePrefix, vpcCidr } from "../config";

/** Split VPC CIDR into 4 /22 subnets (e.g. 172.16.0.0/20 -> 4 subnets). */
function subnetCidrsFromVpcCidr(cidr: string): [string, string, string, string] {
  const [base, prefixLen] = cidr.split("/");
  const len = parseInt(prefixLen ?? "20", 10);
  if (len >= 22) {
    return ["10.0.0.0/22", "10.0.4.0/22", "10.0.8.0/22", "10.0.12.0/22"];
  }
...
const vpc = new aws.ec2.Vpc("CoreVpc", {
  cidrBlock: vpcCidr,
